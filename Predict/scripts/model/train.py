...
parse.add_argument('-nb_flow2', type=int, default=1)
...
def train():
    ...
    for i in range(opt.epoch_size):
        ...
        if valid_loss[-1] < best_valid_loss:
            ...
            # print("model_filename:",opt.model_filename)
...
def predict(test_type='train'):
    ...
    with torch.no_grad():
        for idx, (c, target) in enumerate(data):
            ...
            # attn_t_list.append(attn_t)
            # print(pred.data.cpu().size(),target.data.size())
            loss.append(criterion(pred, y).item())
    final_predict = np.concatenate(predictions)
    ground_truth = np.concatenate(ground_truth)
    # attn_s = np.concatenate(attn_s_list)
    # attn_t = np.concatenate(attn_t_list)
    print(
...
if __name__ == '__main__':
    ...
    with open(path_name + '/result.txt', 'a') as f:
